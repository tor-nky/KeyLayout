...
新MS-IME（Windows 10 version 2005）では、設定を初期状態にします。（旧MS-IME もこれで動きます）

他の IME の場合を、ATOK を例に説明します。
...
* 新MS-IME（Windows 10 version 2005）では、左右シフトを押しっぱなしにしないと、一時的な英数入力がうまく動きません。
...
### 新MS-IME（Windows 10 version 2005）に対応させるため、ローマ字入力化。
...
この状態でもかな入力が可能です。（旧EDGE に対応しています）
