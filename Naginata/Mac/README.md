...
英語系の配列を取り除いかなくてはいけませんが、英語系の配列は Excelワークシートの書き換えてスクリプトを作成すれば、実装できます。

配列は、パスワード入力時は無効、Command、Option、Controlキーと併用時も無効です。
...
Mac では「あ」と「ぁ」のような小書き文字との区別がつかない問題がありますが、ワークシート内の手動設定で回避しています。
...
A6. Excel ワークシートの「キー設定」タブの JISキーボード の文字を入れ替え、上の方法でスクリプトを作成して登録してください。
