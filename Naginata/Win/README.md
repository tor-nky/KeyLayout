...
Shift+[ 、Shift+] で『』を入力したあとは文字確定になります。
...
* キーの押したときに、変換せずにそのまま通ってしまうことがある。また主にスペースキーだが、キーが押されたままになる。

IME によって頻度が変わりますが、全く起こらない IME はありません。

以前気になって数えた結果は、次の通りです。

旧MS-IME：ごくわずか。40分×3セットに1回起きるかどうか。

新MS-IME：数分〜数十分に1、2回程度。

ATOK：悪い時は1分に3回起きる。良い時は40分使って1度も起きない。

* スタートメニューの検索、エクスプローラの検索で記号入力しても、その前に入力した文字が自動確定されない。

未確定の文字があることを検知できないからです。

* 新MS-IME で、かな変換中に英数入力に切り替え、確定しないでキーを押すと最初の文字が入力されない。

新MS-IME の仕様です。

* 新MS-IME で記号入力すると、意図せず改行されることがある。

* ATOK で M+Comma+B 「　　　×　　　×　　　×{改行}」の入力に時間がかかり、かつ連打ができない。

半角英数モードなら、高速かつ連打できます。
